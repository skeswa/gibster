...
class TestUserEndpoints:
    ...
    def test_update_credentials_success(self, client, sample_user_data, sample_credentials):
        """Test successful credential update"""
        headers = self.setup_authenticated_user(client, sample_user_data)
        
        response = client.put("/api/v1/user/credentials", 
                            json=sample_credentials, 
                            headers=headers)
    ...
    def test_sync_bookings_with_credentials(self, client, sample_user_data, sample_credentials):
        """Test sync with Gibney credentials (will fail without real login)"""
        headers = self.setup_authenticated_user(client, sample_user_data)
        
        # Add credentials first
        client.put("/api/v1/user/credentials", 
                  json=sample_credentials, 
                  headers=headers)
...
class TestCalendarEndpoints:
    ...
    def test_calendar_feed_valid_uuid_no_bookings(self, client, sample_user_data, test_db):
