...
def check_virtual_env():
    ...
    in_venv = (
        ...
        or sys.executable.startswith(os.path.join(os.getcwd(), "venv"))
        or sys.executable.startswith(os.path.join(os.getcwd(), ".venv"))
    )
    ...
    venv_python_but_no_activation = (
        ...
        or sys.executable.startswith(os.path.join(os.getcwd(), ".venv"))
    ) and os.environ.get("VIRTUAL_ENV") is None

    if venv_python_but_no_activation:
        print(
            "⚠️  Warning: Running with venv Python but environment not fully activated!"
        )
...
def run_frontend_type_check(args):
    ...
    if not os.path.exists(tsconfig_path):
        print(
            f"⚠️  No tsconfig.json found in '{frontend_dir}', skipping frontend type checking"
        )
        return 0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run Gibster tests with optional type checking"
    )
    parser.add_argument(
        "--type",
    ...
    parser.add_argument(
        "--skip-type-check",
        action="store_true",
        help="Skip type checking (type checking is on by default)",
    )
    parser.add_argument(
        "--type-check-only",
        action="store_true",
        help="Run only type checking, no tests",
    )
    parser.add_argument(
        "--e2e",
        action="store_true",
        help="Run end-to-end tests against real Gibney site (requires credentials)",
    )
    ...
    if total_exit_code == 0:
        ...
        if type_check_failed:
            ...
            if not args.skip_type_check:
                ...
                frontend_offset = (
                    3 if not args.backend_only and not args.frontend_only else 1
                )
            else:
                backend_offset = 0
                frontend_offset = 1
            ...
            if (
                not args.frontend_only
                and len(exit_codes) > backend_offset
                and exit_codes[backend_offset] != 0
            ):
                print("   - Backend tests failed")
            if (
                not args.backend_only
                and len(exit_codes) > frontend_offset
                and exit_codes[frontend_offset] != 0
            ):
